...
CREATE TABLE Questions (
    ...
    HasImageOptions BIT DEFAULT 0 -- Флаг, указывающий, есть ли у вопроса варианты с картинками
)
GO
...
CREATE TABLE AnswerOptions (
    ...
    ImagePath NVARCHAR(200), -- Путь к изображению варианта ответа
...
INSERT INTO Questions (QuestionText, OrderNumber, HasImageOptions) VALUES 
(N'Какой тип отдыха Вас больше привлекает?', 1, 1),
(N'С кем вы планируете отправиться в тур?', 2, 1),
(N'Какой бюджет у Вас запланирован на тур?', 3, 0),
(N'Какие виды транспорта вы рассматриваете для путешествия?', 4, 1),
(N'В какое время года вы планируете свой отдых?', 5, 1),
(N'Какие условия по проживанию?', 6, 1),
(N'На какой уровень в отеле вы ориентируетесь?', 7, 1),
(N'Предпочитаете ли Вы путешествовать самостоятельно или предпочитаете групповые туры с гидом?', 8, 0),
(N'Какое количество времени вы готовы уделить путешествию?', 9, 0),
(N'Планируете ли вы заниматься какой-либо спортивной или экстремальной деятельностью во время путешествий?', 10, 1)
GO

-- Добавление тестовых данных: Варианты ответов с изображениями
